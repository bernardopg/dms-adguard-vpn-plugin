...
Esta hotfix elimina um falso positivo persistente que bloqueava conexões TUN no startup e após desconectar em sistemas com mais de uma interface física.

### Destaques

- Corrigida a checagem de múltiplas rotas default que bloqueava TUN quando Ethernet e Wi-Fi tinham rotas default com métricas diferentes.
- A detecção agora conta apenas rotas que compartilham a menor métrica.
- Interfaces virtuais (`lo`, `docker*`, `veth*`, `br-*`, `virbr*`, `dummy*`) são ignoradas.
- Adicionado toggle **Bypass Multi-Route Check** em Settings → Advanced.
