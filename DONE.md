...
- [x] **Indicador visual de ping** — ✅ 2026-07-10: `pingToneColor()` novo em `AdGuardVpnWidget.qml` (verde `Theme.success` <80ms, amarelo `Theme.warning` <150ms, vermelho `Theme.error` ≥150ms, neutro `Theme.surfaceVariantText` quando ping desconhecido/`-1`) aplicado no fundo (alpha 0.12) e borda (alpha 0.4) do badge de ping em cada card de localização, sem alterar o texto/formatação existente.
